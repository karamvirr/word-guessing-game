...
if (slug && slug.length > 0) {
  const roomChannel = consumer.subscriptions.create({ channel: 'RoomChannel', slug: slug }, {
    ...
    updatePlayerRoster(data) {
      ...
      data.users.forEach((element, index) => {
        let playerCardHTML = `
          <li class="player-card u-py16" id="${element.id}">
            <div>
              <p style="color: gray;" class="u-mr8">${index + 1}</p>
              <p>
                <b>${element.name} <span>${((current_user_id === element.id) ? '(you)' : '')}</span></b>
                <br>
                <span style="font-size: 14px;">${element.score} PTS</span>
              </p>
            </div>
  ...
  function emitDrawing() {
    ...
    // Calls 'RoomChannel#draw(data)' on the server.
